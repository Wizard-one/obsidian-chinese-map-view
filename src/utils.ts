...
/*
 * Coordinate conversion: GCJ-02 (used by AMap/Gaode) to WGS-84 (used by Leaflet/OSM).
 * Based on the standard conversion algorithm.
 */
const PI = Math.PI;
const A = 6378245.0;
...
/**
 * Converts GCJ-02 (AMap/Gaode) to WGS-84 (standard GPS / Leaflet).
 * @returns [longitude, latitude] in WGS-84
 */
export function gcj02ToWgs84(lng: number, lat: number): [number, number] {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        const dLatCorrected = (dLat * 180.0) / (((A * (1 - EE)) / (magic * sqrtMagic)) * PI);
        const dLngCorrected = (dLng * 180.0) / ((A / sqrtMagic) * Math.cos(radLat) * PI);
        wgsLng = lng - dLngCorrected;
        wgsLat = lat - dLatCorrected;
    }
